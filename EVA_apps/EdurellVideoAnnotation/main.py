...
def video_selection():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # [TODO] or maybe insert here video analysis
            segmentation_data = db_mongo.get_video_segmentation(vid_id)
            if segmentation_data is not None:
                print("skipping segmentation")
            else:
                is_slide_video = vid_analyzer.is_slide_video()
                print('loading video data')
                db_mongo.insert_video_text_segmentation({'video_id':vid_id,'is_slide_video':is_slide_video}) 
                print('loaded')
            if False and is_slide_video:
...
def burst_launch():
    ...
    if graphs is not None:
        ...
        #[NOTE] matching on user complete name is not safe, id should be passed 
        # but i cannot find source for that info
        annotators = graphs["annotators"]
        my_name = current_user.complete_name
        indx_annotator = 0
        for i,annot in enumerate(annotators):
            if annot['name']==my_name:
                indx_annotator = i
                break
