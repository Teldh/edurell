    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if segmentation_data is None:
                # if there's no segmentation segment the video
                video_slidishness,slide_frames = vid_analyzer.is_slide_video(return_value=True)
                segmentation_data = {'video_id':vid_id,'video_slidishness':video_slidishness,'slide_frames':slide_frames}
            else:
                video_slidishness,slide_frames = segmentation_data['video_slidishness'],segmentation_data['slide_frames']
            if vid_analyzer.is_slide_video(video_slidishness):
                vid_analyzer.analyze_video()
                results = vid_analyzer.extract_titles()
                # insert titles data in element that will be loaded on the db
                segmentation_data = {**segmentation_data, **{'slide_titles':[{'start_end_seconds':start_end_seconds,'title':title,'xywh_normalized':bb} for (title,start_end_seconds,bb) in results]}}
            db_mongo.insert_video_text_segmentation(segmentation_data)
            if False and vid_analyzer.is_slide_video():
                # Can't be done in this way, requires a more structured code a priori otherwise thumbnails glitch.
                # [BUG] the problem is that thumbnails of keyframes are entangled with transcript segmentation thus cannot be replaced with ease 
...
@app.route('/download_burst_json', methods=["GET", "POST"])
def download_burst_json():

    print("***** EDURELL - Video Annotation: main.py::download_burst_json(): Inizio ******")
    ...
    print("***** EDURELL - Video Annotation: main.py::download_burst_json(): Fine ******")
    ...
    if form.validate_on_submit():
        ...
        for i,annot in enumerate(annotators):
            if annot['id']==my_id:
                ...
                indx_annotator = i
                break
        k = get_real_keywords(video_id, graphs, annotators[indx_annotator]['id'],title=True)
        if k is None:
...
def burst_launch():
    ...
    if syn_burst:
        ...
        if burstType == 'auto':
            ...
            if vid_analyzer.is_slide_video(segmentation_data['video_slidishness']):
                titles = segmentation_data['slide_titles']
                defs, in_depths = vid_analyzer.get_definitions_and_indepth_times(titles)
        else:    
            concept_map, definitions = burst_extraction(video_id, concepts)
    ...
    if graphs is not None:
        ...
        json["agreement"] = {
            "name":"You", #graphs["annotators"][indx_annotator]["name"],
