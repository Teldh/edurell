...
export default class Graph extends React.Component {
     ...
     * @param  {string} source - id of the source node that participates in the event.
     * @param  {string} target - id of the target node that participates in the event.
     * @param {string} associationType - id of the association between nodes that participates in the event.
     * @returns {undefined}
     */
    onMouseOverLink = (source, target, associationType) => {
        this.props.onMouseOverLink && this.props.onMouseOverLink(source, target, associationType);

        if (this.state.config.linkHighlightBehavior) {
            this.state.highlightedLink = { source, target, associationType };
    ...
    render() {
        ...
        return (
            <div style={{"height":"100%", "width":"100%"}} id={`${this.state.id}-${CONST.GRAPH_WRAPPER_ID}`}>

                <svg style={{"height":"99%", "width":"99%"}}>
                    <defs>
                    <marker id="TriangleEnd" viewBox="0 0 10 10" fill="blue" refX="19" refY="5"
                            markerUnits="strokeWidth" markerWidth="15" markerHeight="8" orient="auto">
                        <path d="M 0 0 L 10 5 L 0 10 z" />
                    </marker>
                    <marker id="TriangleStart" viewBox="0 0 10 10" fill="#3182bd" refX="-9" refY="5"
                            markerUnits="strokeWidth" markerWidth="8" markerHeight="15" orient="auto">
                        <path className="marker" d="M 10 0 L 0 5 L 10 10 z" />
                    </marker>
